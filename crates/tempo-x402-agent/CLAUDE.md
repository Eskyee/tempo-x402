...
9-step clone workflow: create service → set env vars → set Docker image → add volume → set resource limits → create domain → deploy. All via Railway GraphQL API.
...
- `CloneConfig.child_env_vars` injects extra env vars (soul config, API keys) into children
- `CloneConfig.clone_cpu_millicores` / `clone_memory_mb` set resource limits via `update_service_resources()`
