...
/// Fetch dev report — structured diagnostics for identifying code improvements
pub async fn fetch_dev_report() -> Result<serde_json::Value, String> {
    let resp = Request::get(&format!("{}/soul/dev-report", GATEWAY_URL))
        .send()
        .await
        .map_err(|e| format!("Request failed: {}", e))?;
