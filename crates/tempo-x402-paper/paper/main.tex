...
Because \texttt{cargo test} is a staged pipeline (parse $\to$ type-check $\to$ borrow-check $\to$ run tests), we can classify failures by the stage at which they occur. Table~\ref{tab:failmode} does this by grepping the captured error output for diagnostic signatures.

\begin{table}[h]
\centering
\caption{Failure-mode distribution across configurations. Compile = any \texttt{rustc} error or borrow-checker rejection. Test = code compiled but a \texttt{\#[test]} panicked or failed. Empty = model produced no parseable code. As the model improves, failure mass migrates from compile to test, indicating that fine-tuning teaches syntax/types before semantics.}
\label{tab:failmode}
\begin{tabular}{lrrrrr}
\toprule
Config & Pass & Compile-fail & Test-fail & Empty & Other \\
\midrule
0.5B base        & 5  & 173 & 8  & 11 & 4 \\
0.5B Full SFT    & 33 & 108 & 23 & 24 & 13 \\
3B base          & 54 & 96  & 23 & 24 & 4  \\
3B + LoRA (122)  & 73 & 64  & 30 & 25 & 9  \\
3B + LoRA (178)  & 69 & 54  & 37 & 32 & 9  \\
\bottomrule
\end{tabular}
\end{table}

Two patterns stand out. First, the absolute number of compile-failures falls monotonically across every training step, even where raw pass count regresses (178 vs.\ 122 at 3B). Fine-tuning is reliably teaching the model to produce code that at least type-checks and borrow-checks, which is a structural improvement that is not captured by the pass@1 headline. Second, the \emph{test-failure} count rises monotonically in the same direction. The model is increasingly producing plausible-looking Rust that compiles but has the wrong logic---exactly the population that an FCM-style concept-targeted curriculum (Section~\ref{sec:future}) would be expected to attack.
...
All fine-tuning and inference ran on a single consumer machine: one RTX 3090 (24\,GB VRAM), 128\,GB system RAM. LoRA fine-tunes ran in 5--20 minutes each; the full-SFT 0.5B run in $\sim$45 minutes (4-bit quantized base). Benchmarking each config against the full 201 problems takes 25--35 minutes with the fine-tuned model loaded in \texttt{llama-server} (Q4\_K\_M quantization, CPU inference in our current build). The total compute to reproduce every row of Table~\ref{tab:main} is on the order of 6--8 GPU-hours plus $\sim$4 CPU-hours of benchmarking.

\paragraph{Limitations.}
(i) We evaluate only on \textsc{Opus-201}, which was itself generated by Opus---there is a legitimate concern that the benchmark encodes Opus's implicit style and thus inflates the Opus ceiling. Cross-evaluating on a disjoint Rust benchmark (e.g., Rust problems from MultiPL-E) is future work. (ii) All our fine-tuning is on Qwen2.5-Coder; we do not test whether the self-play effect transfers to DeepSeek-Coder or StarCoder. (iii) The 30-second \texttt{cargo test} budget can mask infinite-loop failures as generic failures in our failure-mode classifier. (iv) The ``Compile-fail'' bucket in Table~\ref{tab:failmode} is classified by string-matching on captured error output; a small number of failures with unusual error shapes may be mis-classified as ``Other''.
...
The benchmark, harness, training scripts, the LaTeX source of this paper, and all intermediate result JSON files live in the same repository. The numerical results in Table~\ref{tab:main} and Table~\ref{tab:failmode} are produced by:

\begin{lstlisting}[language=bash]
# 1. Serve a fine-tuned GGUF model via llama.cpp's HTTP server.
llama-server -m selfplay_runs_v2/models/qwen3b_lora16.gguf \
...
cargo run --release --bin paper-bench -- score-local \
    ...
    --name qwen-3b-lora16 \
    --output results/qwen-3b-lora16.json

# 3. Regenerate figures from all result JSONs.
python crates/tempo-x402-paper/scripts/plot_figures.py
\end{lstlisting}

The self-play loop that produced the training sets (\texttt{train.jsonl}, $n=122$ and $n=178$) is driven by \texttt{run\_selfplay\_v2.sh}; that script is deterministic modulo sampling temperature and time-to-compile variance. A version-stamped release of the \textsc{Opus-201} problem set and reference results is pinned to the commit referenced by this submission.
