...
impl GenePool {
    ...
    /// Returns (crossovers_created, mutations_created, templates_pruned).
    pub fn evolve(&mut self) -> (usize, usize, usize) {
        let now = chrono::Utc::now().timestamp();
        self.generation += 1;
...
mod tests {
    ...
    fn test_crossover() {
        ...
        let (crossovers, mutations, _) = pool.evolve();
    ...
    fn test_evolution_cycle() {
        ...
        let (crossovers, mutations, pruned) = pool.evolve();
        assert_eq!(pool.generation, 1);
        // Evolution should have done something
        assert!(
            crossovers + mutations + pruned >= 0,
            "Evolution should process without panic"
        );
    }
