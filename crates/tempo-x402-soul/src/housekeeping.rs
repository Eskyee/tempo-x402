...
/// Without this, brain weights, cortex state, codegen models, peer data,
/// and benchmark caches grow forever and fill the disk.
    ...
    for &(key, max_bytes) in large_blob_caps {
        if let Ok(Some(val)) = db.get_state(key) {
            if val.len() > max_bytes {
                tracing::warn!(
                    key = key,
                    size_kb = val.len() / 1024,
                    cap_kb = max_bytes / 1024,
                    "Pruning oversized soul_state blob"
                );
                freed_bytes += val.len();
...
fn cleanup_disk(workspace_root: &str) {
    ...
    // cargo check creates 2-4GB of artifacts. Do NOT wait for a size threshold.
    let target_dir = format!("{workspace_root}/target");
    if std::path::Path::new(&target_dir).exists() {
        let size = dir_size_mb(&target_dir).unwrap_or(0);
        tracing::info!(size_mb = size, "Disk cleanup: removing workspace target/");
        let _ = std::fs::remove_dir_all(&target_dir);
    }

    // 2. Clean cartridge build targets — always remove, they're regenerable
    ...
    // 4. Git garbage collection — aggressive to prevent .git bloat
